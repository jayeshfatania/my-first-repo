...
| T12 | Security and penetration test completed | ⬜ Not started | Developer / Owner | Must have | OWASP Top 10 review required as a minimum. Professional penetration test required before go-live - not optional once Firebase is live and real user data is in Firestore. Scope must include: XSS via dynamic HTML insertion, Firestore security rules, Firebase Auth flows, Firebase Storage rules, Cloudflare Worker proxy, and any remaining exposed API keys (H10). Run against production URL (sniffout.app), not localhost. |
...
| T16 | Full end-to-end test pass completed | ⬜ Not started | Developer / Designer / Owner | Must have | A coordinated full test pass across all tabs, all user flows, and all device types before beta launch. Scope: all five tabs (Today, Weather, Walks, Nearby, Me), all Me tab subpages (Walk Journal, Badges, Saved Walks, Recently Viewed), dog profile creation and editing, walk logging (curated and custom), all Settings options, dark mode, offline behaviour, PWA install flow. Test on both iOS Safari and Android Chrome. This is a separate pass from individual feature testing - it is a full regression check before any real users are invited. |
| T17 | Firebase security review completed | ⬜ Not started | Developer / Owner | Must have | Dedicated security review of all Firebase components before Phase 3 goes live. Scope: Firestore security rules (ensure users can only read and write their own documents), Firebase Storage rules (ensure users can only access their own files), Firebase Auth configuration (anonymous auth settings, auto-deletion disabled per D4), account linking flows tested for edge cases, right to erasure tested end to end. This review is separate from T12 (general pen test) and must be completed before the Phase 3 migration runs for any real users. Gated on Phase 3 build being complete. |
...
5. **T12** — Security and penetration test not completed. Required before go-live once Firebase is live and real user data is in Firestore.
6. **T16** — Full end-to-end test pass not completed. Required before any real users are invited.
7. **T17** — Firebase security review not completed. Required before Phase 3 migration runs for any real users.

Note: T1 (API key proxy) and T14 (manifest start_url) were resolved on 19 March 2026 and have been removed from this list. T12 (security and pen test), T16 (end-to-end test pass), and T17 (Firebase security review) added as hard blockers 23 March 2026.
