...
### Google Cloud billing — resolved 25 March 2026

Root cause identified: the Cloudflare Worker was using an API key from the old Walk Planner project. All Places API charges were being billed to Walk Planner, not sniffout.
...
- Green spaces cache extended to 60 min expiry
...
- Active handoff: docs/handoffs/session-handoff-march-25.md (this document)
...
- **24 March** — Design elevation (Fraunces, #2C4A14, hero card, verdict overhaul, Me tab polish, haptics, wind safety fix), Designer brief A and B implemented (FAB, Me tab, walk cards, maps, Today hero, Weather hazard cards, Settings cog), bug fixes, brand guidelines document created, brand colour corrected in CLAUDE.md
...
- Walk window now shows "Tomorrow" label when suggesting next-day times
...
- `docs/specs/brand-guidelines-march-24.md` — consolidated brand reference document created
...
### Walk window hours — constrained to 6am-9pm, with "Tomorrow" label

The best walk window display is constrained to a minimum of 6am and a maximum of 9pm. When the suggested window falls on the next day, it is labelled "Tomorrow". Implemented 25 March 2026.
...
### Google Cloud billing — all Places API charges go to sniffout project

All Places API charges must go to the sniffout project (sniffout-fe976), not Walk Planner. Walk Planner Places API is disabled. Do not create new API keys on Walk Planner. Resolved 25 March 2026.

### Places API caching — do not remove or bypass
...
The Designer recommended removing it but the owner confirmed it must stay. Style as an outlined button, not a text link.

### Map pin tap target — 44px DivIcon

DivIcon extended to 44px x 44px for WCAG compliance. Visual circle remains 28px default / 36px selected. Do not reduce the tap target.
...
31. **Google Cloud billing resolved 25 March 2026** — root cause was Walk Planner API key in Worker. New sniffout key in place. Walk Planner Places API disabled. Total charge: £85.17 covered by credits.
